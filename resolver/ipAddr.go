...
func NewIpAddr(recordType uint16, externalResovler string) ipAddr {
	return ipAddr{recordType: recordType, externalResolver: externalResovler}
}

func (r ipAddr) Resolve(domain string, client *dns.Client) (string, error) {
	var err error
	var result *dns.Msg
	result, err = helpers.Resolve(domain, r.recordType, client, r.externalResolver)
